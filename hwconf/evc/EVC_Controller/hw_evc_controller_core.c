...
void hw_init_gpio(void) {
	...
	//ESTOP
	palSetPadMode(HW_ESTOP_GPIO, HW_ESTOP_PIN, PAL_MODE_INPUT_PULLUP);

	//PRECHARGE
	palSetPadMode(HW_PRECHARGE_GPIO, HW_PRECHARGE_PIN, PAL_MODE_INPUT_PULLUP);
	...
	// ENABLE_GATE
#ifdef HW_EVC_CONTROLLER_IS_VEDDER_FIRST_PCB 
	...
	palSetPadMode(GPIOB, 5,
			...
			PAL_STM32_OSPEED_HIGHEST);
#endif
	if (palReadPad(HW_ESTOP_GPIO, HW_ESTOP_PIN) == PAL_LOW) {
		ENABLE_GATE();
	}
	// Current filter
...
#if defined(HW_EVC_CONTROLLER_IS_MK6) && defined(HW_EVC_CONTROLLER_IS_MAX)
...
#endif
	if (GET_INPUT_VOLTAGE() < 2) {
		palSetPadMode(HW_PRECHARGE_GPIO, HW_PRECHARGE_PIN, PAL_HIGH);
		if (GET_INPUT_VOLTAGE() > 40) {
			//do stuff;
	} else {
